...
pipeline {
    ...
    stages {
        ...
        stage('Build') {
            steps {
                script {
                    def image = docker.build("ref-app:${BUILD_NUMBER}")
                }
            }
        }
        stage('Test') {
            steps {
                sh 'docker run --rm ref-app:${BUILD_NUMBER} npm test'
            }
        }
        stage('Security Scan') {
            steps {
                sh 'trivy image --severity CRITICAL,HIGH --exit-code 0 ref-app:${BUILD_NUMBER}'
            }
        }
        stage('Push') {
            when {
            ...
            steps {
                withCredentials([usernamePassword(
                    ...
                    sh """
                        ...
                        docker tag ref-app:${BUILD_NUMBER} ${DOCKER_USER}/ref-app:${BUILD_NUMBER}
                        docker push ${DOCKER_USER}/ref-app:${BUILD_NUMBER}
                    """
                }
            }
        }
